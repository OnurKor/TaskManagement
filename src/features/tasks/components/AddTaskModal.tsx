...
const TaskSchema = Yup.object().shape({
  ...
  EstimatedHour: Yup.number()
    ...
    .test(
      "is-decimal",
      "Estimated hours can have up to 1 decimal place",
      (value) =>
        value === undefined ||
        value === null ||
        /^\d+(\.\d{0,1})?$/.test(value.toString())
    ),
  SprintID: Yup.number()
...
const AddTaskModal = ({
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      console.error(
        `Task ${mode === "create" ? "adding" : "updating"} error:`,
        error
      );
      setSubmitError(
        error.message ||
              ...
              <Form>
                ...
                >
                  ...
                  <Box>
                    <TextField
                      ...
                      error={
                        touched.EstimatedHour && Boolean(errors.EstimatedHour)
                      }
                      helperText={
                      ...
                      <Select
                        ...
                        onChange={(e) => {
                          ...
                          const selectedTask = tasks.find(
                            (task) => task.id === Number(selectedValue)
                          );
                          // Eğer parentID varsa (yani bu task da bir alt task ise), uyarı ver
                          if (selectedTask && selectedTask.ParentID) {
                            setSnackbar({
                              open: true,
                              message:
                                "A sub-task cannot be assigned as a parent task.",
                              severity: "warning",
                            });
                            // Seçimi engelle
                            return;
                          }
                          setFieldValue("ParentID", Number(selectedValue));
                        }}
                        disabled={isSubmitting || tasks.length === 0}
                        label="Parent Task (Optional)"
                        sx={{
                ...
                >
                  ...
                  >
                    ...
                    Fields marked with <span className="required-icon">*</span>{" "}
                    are required
                  </Typography>
                  ...
                  >
                    After the task is created, assigned users will be able to
                    track their tasks
                  </Typography>
                </Box>
                ...
                <DialogActions sx={{ px: 0, pb: 0, pt: 3 }}>
                  ...
                  <Button
                    ...
                    sx={{
                      ...
                      background:
                        !isSubmitting && (dirty || mode === "create")
